...
const Navbar = () => {
  ...
  const navLinks = isAuthenticated 
    ? isAdmin 
      ? [...publicNavLinks.filter(link => link.name !== 'Login' && link.name !== 'Sign Up'), { name: 'Admin Dashboard', path: '/admin-dashboard' }]
      : [...publicNavLinks.filter(link => link.name !== 'Login' && link.name !== 'Sign Up')]
    : publicNavLinks;
            ...
            <div className="flex items-center space-x-4">
              {isAuthenticated ? (
                ...
                <div className="hidden sm:flex items-center space-x-3">
                  <span className="text-white/80 text-sm">Welcome to Hykoo Events</span>
                </div>
              )}
            ...
            <div className="flex flex-col space-y-1 pb-4">
              {navLinks.map((link) => (
                <button
                  key={link.path}
                  onClick={() => handleNavigationClick(link.path)}
                  className={`px-4 py-3 text-white rounded-lg transition-colors text-left ${
                    isActive(link.path)
                      ? 'bg-white/20'
                      : 'hover:bg-white/10'
                  }`}
